...
namespace ARSoft.Claim.DAL.Master
{
    public class UserControlDAL
    {
        ...
        {
            return ctx.uspGetAuthorizedMenus(userName, parentID, levels, types, withSystem, currentUrl).ToList();
        }

        public List<uspGetPermissions_Result> GetPermissionEntityList(string userName, string functionCode, int? MasterID)
        {
            return ctx.uspGetPermissions(userName, functionCode, MasterID).ToList();
        }

        public List<uspGetUserRoles_Result> GetUserRolesEntityList(string userName)
        {
            return ctx.uspGetUserRoles(userName).ToList();
        }
        ...
        public PermissionModel ConvertToPermissionModel(uspGetPermissions_Result permissionEntity)
        {
            ...
            permissionModel.ID = permissionEntity.ID;
            permissionModel.CAPTION = permissionEntity.CAPTION;


            //Tempolary  Work aronbd before Change Store Procedure 
            permissionModel.URL = ctx.SYS_COMPONENTS.Where(t => t.ID == permissionEntity.ID).FirstOrDefault().URL;
            permissionModel.FunctionCode = permissionEntity.FunctionCode;
            permissionModel.FunctionCaption = permissionEntity.FunctionCaption;
            permissionModel.LEVELS = permissionEntity.LEVELS;
            permissionModel.SCREENSEQUENCE = permissionEntity.SCREENSEQUENCE;
            permissionModel.Search = permissionEntity.Search;
            permissionModel.Add = permissionEntity.Add;
            permissionModel.Edit = permissionEntity.Edit;
            permissionModel.Delete = permissionEntity.Delete;
            permissionModel.View = permissionEntity.View;
        ...
        public string GetAllMenuRenderText(UserAuthorizationModel userName)
        {
            ...
            retrunString += " <li class=\"header\">MAIN NAVIGATION</li>";
            ...
            foreach (MenuModel lv1 in level1AuthorizedMenuModelList)
            {
                ...
                {
                    retrunString += GetMenuRenderString(lv1);
                }
                else
                {
                    retrunString += GetParentMenuRenderString(lv1, level2AuthorizedMenuModelList);
                }
            }
        ...
        public string GetMenuRenderString(MenuModel menumodel)
        {
            string retrunString = string.Empty;

            retrunString += "<li>";
            retrunString += "<a href=\"..\\@URL\">";
            retrunString += "<i class=\"fa fa-th\"></i>@Caption</span>";
            retrunString += "</a>";
            retrunString += "</li>";
        ...
        public string GetParentMenuRenderString(MenuModel menumodel, List<MenuModel> level2AuthorizedMenuModelList)
        {
            string retrunString = string.Empty;
            ...
            retrunString += "<span>@Caption</span>";
            ...
            retrunString = retrunString.Replace("@Caption", menumodel.Caption);
            retrunString = retrunString.Replace("@childMenu", GetAllChildMenuRenderString(level2AuthorizedMenuModelList));

            return retrunString;
        }

        public string GetAllChildMenuRenderString(List<MenuModel> level2AuthorizedMenuModelList)
        {
            string retrunString = string.Empty;
            ...
            {
                retrunString += GetChildMenuRenderString(lv2);
            }
        ...
        public string GetChildMenuRenderString(MenuModel menumodel)
        {
            string retrunString = string.Empty;

            retrunString += "<li>";
            retrunString += "<a href=\"..\\@URL\">";
            retrunString += "<i class=\"fa @Icon\"></i><span>@Caption</span>";
            retrunString += "</a>";
            retrunString += "</li>";
